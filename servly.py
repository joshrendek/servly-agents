...
#### ArchLinux 
# On a fresh install make sure you: pacman --sync --refresh
#--- then pacman -S python
#--- then pacman -S sysstat
#### Fedora
#--- yum install sysstat
#### Other Linux
#--- make sure the sysstat package is installed
#--- and that sar is available to be executed
...
if len(release_info) == 0:
    release_info += os.popen("cat /etc/debian_version").readline()
...
# get num of cpus
ncpus = int(os.popen("cat /proc/cpuinfo | grep -c 'processor'").readline())

# cpu usage
usage = 0
cpu_free = os.popen("sar -u 30 1 | awk '{ print $NF }' | tail -n 1").readline()

# lets get network usage now via sar, we need to figure out which columns are what and sum them
net = commands.getoutput("sar -n DEV 1 1").split('\n')
...
#package updates
update_cnt = int(os.popen("yum check-update | grep -v 'Loading mirror' | grep -v 'Loaded plug' | grep -v ' \* ' | wc -l").readline())
