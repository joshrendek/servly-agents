...
############### INSTALLATION NOTES ###############
#### Packages/Steps to install
#--- cd /usr/ports/lang/python; make && make install

#--- cd /usr/ports/sysutils/bsdsar; make && make install
#--- cd /usr/ports/sysutils/muse; make && make install

### PERL

#### FREEBSD 7
#--- cd /usr/ports/lang/perl5; make && make install
#--- ln -s /usr/local/bin/perl /usr/bin/perl

#### FREEBSD 8
#--- cd /usr/ports/lang/perl5.8; make && make install


# tested on: FreeBSD 7.x
# Need to install Python + Sar
# /usr/ports/lang/python & /usr/ports/sysutils/bsdsar
# you also need to make sure perl works and is in /usr/bin/perl
# for some reason bsd installs it in /usr/local/bin but bsdsar looks in 
# /usr/bin
# Also for memory reporting to work properly you need to install muse
# /usr/ports/sysutils/muse
# that should be it :) 
# lots of abs paths were required as cron didnt pick up PATHs properly
# may need to adjust depending on your bsd env
...
free_memory = 0
used_memory = 0
free = os.popen("/usr/local/bin/muse | tail -n 1 | awk '{print $2}'").readline()
total = os.popen("/usr/local/bin/muse | tail -n 2 | head -n 1 | awk '{print $2}' ").readline()

free_memory = float(free) # already in bytes
used_memory = float(total)-free_memory # already in bytes

# get num of cpus
ncpus = int(os.popen("sysctl hw.ncpu | awk '{ print $2 }'").readline())

# cpu usage
# windows: http://kahrn.pastebin.com/f2430d665
# linux: ps -e -o pcpu
usage = 0
cpu_free = os.popen("/usr/local/bin/bsdsar -u 30 | awk '{ print $NF }' | tail -n 1").readline()

# lets get network usage now via sar, we need to figure out which columns are what and sum them
net = commands.getoutput("/usr/local/bin/bsdsar -I").split('\n')
